<template>
  <div class="baniya-node" :style="{ '--node-color': nodeColor }">
    ...
    <div class="node-body">
      <div
        v-for="preview in configPreviews"
        :key="preview.key"
        class="node-preview"
      >
    ...
    />
    <Handle
      v-for="(h, i) in outputHandles"
      :key="h.id"
      type="source"
      :id="h.id"
      :position="Position.Right"
      :style="{ top: getHandleTop(i, outputHandles.length) }"
    />
  </div>
</template>
...
const ICONS: Record<string, string> = {
  ...
  'ai.llm':
    'M12 2a2 2 0 0 1 2 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 0 1 7 7h1a1 1 0 0 1 0 2h-1.27a7 7 0 0 1-13.46 0H5a1 1 0 0 1 0-2h1a7 7 0 0 1 7-7h1V5.73A2 2 0 0 1 12 2z',
  'ai.classify':
  ...
  'data.set':
    'M11 5H6a2 2 0 0 0-2 2v11a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2v-5 M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z',
...
const HANDLE_DEFS: Record<
  string,
  { inputs: { id: string }[]; outputs: { id: string; label: string }[] }
> = {
  'ai.classify': {
    inputs: [{ id: 'in' }],
    outputs: [
...
const nodeIcon = computed(
  () => ICONS[props.data.type] || ICONS['ai.llm'] || ''
);
...
const outputHandles = computed(() => {
  ...
  if (props.data.type === 'output.response' || props.data.type === 'output.log')
    return [];
...
const executionCost = computed(() => {
  return props.data.executionCost ?? 0;
});
