    ...
    <table class="audit-table">
      ...
      <tbody>
        <tr v-for="row in sorted" :key="row.id">
          ...
          <td>
            <Badge
              :text="row.sensitivityLevel"
              :color="sensitivityBadgeColor(row.sensitivityLevel)"
            />
          </td>
          <td>
            <Badge
              :text="row.routingDecision"
              :color="routeBadgeColor(row.routingDecision)"
            />
          </td>
          <td>{{ row.modelUsed }}</td>
...
const sorted = computed(() => {
  ...
  if (filterSensitivity.value)
    r = r.filter(x => x.sensitivityLevel === filterSensitivity.value);
  return [...r].sort((a, b) => {
