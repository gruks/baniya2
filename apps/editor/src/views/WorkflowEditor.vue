    ...
    <div class="editor-layout">
      ...
      <div class="editor-toolbar">
        <div class="toolbar-left">
          ...
          <span :class="['badge', workflow?.active ? 'badge-success' : 'badge-muted']">
            {{ workflow?.active ? 'Active' : 'Inactive' }}
          </span>
        </div>
        <div class="toolbar-right">
          <span v-if="saving" class="toolbar-status">Saving...</span>
          <span v-else-if="runStatus === 'running'" class="toolbar-status run-status-running">
            <span class="status-spinner"></span> Running...
          </span>
          <span v-else-if="runStatus === 'success'" class="toolbar-status run-status-ok">✓ Completed</span>
          <span v-else-if="runStatus === 'error'" class="toolbar-status run-status-err">✕ Failed</span>
          <span v-else class="toolbar-status" style="color: var(--color-success);">✓ Saved</span>
          <button class="btn btn-sm btn-secondary" @click="saveWorkflow" :disabled="saving">Save</button>
          <button
      ...
      <div class="editor-panels">
        ...
        <div class="node-picker">
          ...
          <div v-for="cat in categories" :key="cat.name" class="picker-category">
            ...
            <div
              v-for="node in cat.nodes" :key="node.type"
              class="picker-node"
          ...
          <div v-if="lastExecution && !selectedNode" class="exec-result-panel">
            <div class="config-header">
              ...
              <div style="display:flex;gap:6px;align-items:center;">
                <span :class="['badge', lastExecution.status === 'success' ? 'badge-success' : 'badge-error']">
                  {{ lastExecution.status }}
                </span>
                <button class="config-close" @click="lastExecution = null">✕</button>
              </div>
            </div>
            ...
            <div class="exec-summary-bar">
              <div class="exec-stat">
                ...
                <span class="exec-stat-val">{{ lastExecution.totalLatencyMs }}ms</span>
              </div>
              <div class="exec-stat">
                <span class="exec-stat-label">Cost</span>
                <span class="exec-stat-val">${{ lastExecution.totalCostUSD?.toFixed(5) }}</span>
              </div>
              <div class="exec-stat">
                <span class="exec-stat-label">Nodes</span>
                <span class="exec-stat-val">
                  {{ lastExecution.nodeResults?.filter((r: any) => r.status === 'success').length }}✓
                  <span v-if="lastExecution.nodeResults?.filter((r: any) => r.status === 'error').length" class="err-count">
                    {{ lastExecution.nodeResults?.filter((r: any) => r.status === 'error').length }}✕
                  </span>
                </span>
              </div>
            </div>
              ...
              >
                ...
                <div class="exec-node-header">
                  <span :class="['exec-status-icon', `icon-${r.status}`]">
                    {{ r.status === 'success' ? '✓' : r.status === 'error' ? '✕' : r.status === 'running' ? '…' : r.status === 'skipped' ? '—' : '·' }}
                  </span>
                  <span class="exec-node-label">{{ nodeLabel(r.nodeId) }}</span>
                  <span class="exec-node-type">{{ nodeType(r.nodeId) }}</span>
                  <span v-if="r.llmMeta" class="exec-node-cost">${{ r.llmMeta.costUSD?.toFixed(5) }}</span>
                  <span class="exec-node-time">{{ nodeMs(r) }}ms</span>
                  <span class="exec-expand-icon">{{ expandedNodes.has(r.nodeId) ? '▲' : '▼' }}</span>
                </div>
                ...
                <!-- Expanded detail -->
                <div v-if="expandedNodes.has(r.nodeId)" class="exec-node-detail">
                  ...
                  <div v-if="r.output && Object.keys(r.output).length" class="exec-output-block">
                    <div class="exec-output-label">Output</div>
                    <pre class="exec-output-pre">{{ formatOutput(r.output) }}</pre>
                  </div>
                  <div class="exec-timing">
                    Started {{ formatTime(r.startedAt) }} · Finished {{ formatTime(r.finishedAt) }}
                  </div>
                </div>
              </div>
            </div>
          ...
          <template v-if="selectedNode">
            ...
            <div class="config-body">
              ...
              <div v-for="field in getConfigFields(selectedNode.data.type)" :key="field.key" class="form-group">
                ...
                <input v-else-if="field.type === 'password'" type="password" v-model="selectedNode.data.config[field.key]" :placeholder="field.placeholder" @change="autoSave" autocomplete="off" />
                <!-- Folder path field: native OS picker button -->
                <div v-else-if="field.key === 'folderPath'" class="folder-path-row">
                  <input
                    v-model="selectedNode.data.config[field.key]"
                    :placeholder="field.placeholder"
                    class="folder-path-input"
                    @change="autoSave"
                  />
                  <button
                    class="btn btn-sm btn-secondary folder-browse-btn"
                    :disabled="browsingFolder"
                    @click="browseFolder(field.key)"
                    title="Open folder picker"
                  >
                    <span v-if="browsingFolder" class="browse-spinner"></span>
                    <span v-else>📁</span>
                  </button>
                </div>
      ...
      <!-- Run error toast -->
      <div v-if="runError" class="run-toast run-toast-error" @click="runError = ''">
        ✕ {{ runError }}
      </div>
    </div>
  </div>
...
const NODE_REGISTRY: NodeMeta[] = [
  ...
  { type: 'ai.llm', label: 'AI LLM', color: '#0D9E75', category: 'ai', configSchema: [
    { key: 'prompt', label: 'Prompt', type: 'textarea', placeholder: 'Your prompt here. Use {{ input.field }} for data' },
    { key: 'systemPrompt', label: 'System Prompt', type: 'textarea' },
    { key: 'apiProvider', label: 'API Provider', type: 'select', options: [{ label: 'Auto (env keys)', value: 'auto' }, { label: 'OpenAI', value: 'openai' }, { label: 'Anthropic', value: 'anthropic' }, { label: 'Google Gemini', value: 'gemini' }] },
    { key: 'apiKey', label: 'API Key', type: 'password', placeholder: 'sk-... (overrides env variable)' },
  ...
  { type: 'storage.read', label: 'Read File', color: '#0EA5E9', category: 'storage', configSchema: [
    { key: 'path', label: 'File Path', type: 'text', placeholder: 'data/output.txt' },
    { key: 'encoding', label: 'Encoding', type: 'select', options: [{ label: 'UTF-8 (text)', value: 'utf8' }, { label: 'Base64 (binary)', value: 'base64' }] },
  ]},
  { type: 'storage.write', label: 'Write File', color: '#0EA5E9', category: 'storage', configSchema: [
    { key: 'path', label: 'File Path', type: 'text', placeholder: 'data/output.txt' },
    { key: 'content', label: 'Content', type: 'textarea', placeholder: '{{ input.text }}' },
    { key: 'append', label: 'Mode', type: 'select', options: [{ label: 'Overwrite', value: 'overwrite' }, { label: 'Append', value: 'append' }] },
    { key: 'createDirs', label: 'Create Directories', type: 'select', options: [{ label: 'Yes', value: 'true' }, { label: 'No', value: 'false' }] },
  ]},
  { type: 'storage.list', label: 'List Directory', color: '#0EA5E9', category: 'storage', configSchema: [
    { key: 'path', label: 'Directory Path', type: 'text', placeholder: 'data/' },
    { key: 'recursive', label: 'Recursive', type: 'select', options: [{ label: 'No', value: 'false' }, { label: 'Yes', value: 'true' }] },
    { key: 'filter', label: 'Filter (glob)', type: 'text', placeholder: '*.txt' },
  ]},
  { type: 'storage.delete', label: 'Delete File', color: '#0EA5E9', category: 'storage', configSchema: [
    { key: 'path', label: 'File Path', type: 'text', placeholder: 'data/old.txt' },
  ]},
  { type: 'storage.mkdir', label: 'Make Directory', color: '#0EA5E9', category: 'storage', configSchema: [
    { key: 'path', label: 'Directory Path', type: 'text', placeholder: 'data/reports/2024' },
  ]},
  // ─── Folder Nodes ──────────────────────────────────────
  { type: 'folder.connect', label: 'Folder', color: '#EA580C', category: 'folder', configSchema: [
    { key: 'folderPath', label: 'Folder Path', type: 'text', placeholder: 'C:\\Projects\\myapp  or  /home/user/project' },
    { key: 'recursive', label: 'Recursive', type: 'select', options: [{ label: 'No', value: 'false' }, { label: 'Yes', value: 'true' }] },
    { key: 'filter', label: 'Filter (glob)', type: 'text', placeholder: '*.ts' },
  ]},
  { type: 'folder.list', label: 'Folder List', color: '#EA580C', category: 'folder', configSchema: [
    { key: 'folderPath', label: 'Folder Path (override)', type: 'text', placeholder: 'Leave blank to use upstream' },
    { key: 'subPath', label: 'Sub-path', type: 'text', placeholder: 'src/components' },
    { key: 'recursive', label: 'Recursive', type: 'select', options: [{ label: 'No', value: 'false' }, { label: 'Yes', value: 'true' }] },
  ]},
  { type: 'folder.read', label: 'Folder Read', color: '#EA580C', category: 'folder', configSchema: [
    { key: 'folderPath', label: 'Folder Path (override)', type: 'text', placeholder: 'Leave blank to use upstream' },
    { key: 'filePath', label: 'File Path', type: 'text', placeholder: 'src/index.ts  or  {{ input.path }}' },
  ]},
  { type: 'folder.write', label: 'Folder Write', color: '#EA580C', category: 'folder', configSchema: [
    { key: 'folderPath', label: 'Folder Path (override)', type: 'text', placeholder: 'Leave blank to use upstream' },
    { key: 'filePath', label: 'File Path', type: 'text', placeholder: 'output/result.md  or  {{ input.filePath }}' },
    { key: 'content', label: 'Content', type: 'textarea', placeholder: '{{ input.text }}' },
    { key: 'mode', label: 'Mode', type: 'select', options: [{ label: 'Overwrite', value: 'overwrite' }, { label: 'Append', value: 'append' }] },
  ]},
  { type: 'folder.patch', label: 'Folder Patch', color: '#EA580C', category: 'folder', configSchema: [
    { key: 'folderPath', label: 'Folder Path (override)', type: 'text', placeholder: 'Leave blank to use upstream' },
    { key: 'filePath', label: 'File Path', type: 'text', placeholder: 'src/index.ts' },
    { key: 'oldStr', label: 'Old String', type: 'textarea', placeholder: 'Exact text to replace' },
    { key: 'newStr', label: 'New String', type: 'textarea', placeholder: 'Replacement text' },
  ]},
];
...
const { nodeStatuses, connect: wsConnect, resetNodeStatuses, lastEvent } = useWebSocket();
...
async function browseFolder(configKey: string) {
  ...
  try {
    ...
    runError.value = 'Folder picker failed: ' + (e.response?.data?.error || e.message);
...
watch(runStatus, (val) => {
  if (val === 'success' || val === 'error') {
    setTimeout(() => { if (runStatus.value !== 'running') runStatus.value = 'idle'; }, 4000);
  }
});
...
const categories = computed(() => {
  const cats = [
    ...
    { name: 'Storage', nodes: NODE_REGISTRY.filter(n => n.category === 'storage') },
    { name: 'Folder', nodes: NODE_REGISTRY.filter(n => n.category === 'folder') },
...
function onDrop(event: DragEvent) {
  ...
  nodes.value.push({
    ...
    data: { id, type: nodeMeta.type, label: nodeMeta.label, config: {}, disabled: false, executionStatus: 'idle' },
  });
}
...
async function saveWorkflow() {
  ...
  try {
    await store.save(route.params.id as string, { name: workflowName.value, nodes: wfNodes, edges: wfEdges });
  } catch (err: any) {
...
function autoSave() { saveWorkflow(); }
...
<style scoped>
.editor-layout { flex: 1; display: flex; flex-direction: column; overflow: hidden; position: relative; }
...
.toolbar-status { font-size: 12px; color: var(--color-text-muted); display: flex; align-items: center; gap: 4px; }
.run-status-running { color: var(--color-warning); }
.run-status-ok { color: var(--color-success); }
.run-status-err { color: var(--color-error); }
.status-spinner, .btn-spinner {
...
/* Config / Result Panel */
.config-panel { width: 300px; border-left: 1px solid var(--color-border); overflow-y: auto; background: var(--color-bg-primary); display: flex; flex-direction: column; }
.config-header { display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; border-bottom: 1px solid var(--color-border); }
.config-header h3 { font-size: 14px; font-weight: 500; }
.config-close { font-size: 16px; padding: 2px 6px; border-radius: var(--radius-sm); cursor: pointer; }
...
.folder-path-row { display: flex; gap: 4px; align-items: center; }
.folder-path-input { flex: 1; font-size: 12px; min-width: 0; }
.folder-browse-btn { flex-shrink: 0; padding: 4px 8px; font-size: 14px; line-height: 1; }
.folder-browse-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.browse-spinner {
  display: inline-block; width: 10px; height: 10px;
  border: 1.5px solid currentColor; border-top-color: transparent;
  border-radius: 50%; animation: spin 0.6s linear infinite;
...
.exec-result-panel { display: flex; flex-direction: column; height: 100%; overflow: hidden; }
.exec-summary-bar { display: flex; gap: 0; border-bottom: 1px solid var(--color-border); flex-shrink: 0; }
.exec-stat { flex: 1; display: flex; flex-direction: column; align-items: center; padding: 8px 4px; border-right: 1px solid var(--color-border); }
.exec-stat:last-child { border-right: none; }
.exec-stat-label { font-size: 10px; color: var(--color-text-muted); text-transform: uppercase; letter-spacing: 0.04em; }
.exec-stat-val { font-size: 13px; font-weight: 500; margin-top: 2px; }
.err-count { color: var(--color-error); margin-left: 6px; }

.exec-nodes { flex: 1; overflow-y: auto; padding: 6px 0; }
...
.exec-node-card:hover { border-color: var(--color-border-strong); }
.exec-node-success { border-left: 3px solid var(--color-success); }
.exec-node-error   { border-left: 3px solid var(--color-error); }
.exec-node-skipped { border-left: 3px solid var(--color-text-muted); opacity: 0.6; }
.exec-node-running { border-left: 3px solid var(--color-warning); }

.exec-node-header { display: flex; align-items: center; gap: 6px; padding: 7px 10px; font-size: 12px; }
.exec-status-icon { font-size: 11px; font-weight: 700; width: 14px; text-align: center; flex-shrink: 0; }
.icon-success { color: var(--color-success); }
.icon-error   { color: var(--color-error); }
.icon-skipped { color: var(--color-text-muted); }
.icon-running { color: var(--color-warning); }
.exec-node-label { font-weight: 500; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.exec-node-type { font-size: 10px; color: var(--color-text-muted); font-family: monospace; flex-shrink: 0; }
.exec-node-cost { font-size: 10px; color: var(--color-brand); flex-shrink: 0; }
.exec-node-time { font-size: 10px; color: var(--color-text-muted); flex-shrink: 0; }
.exec-expand-icon { font-size: 9px; color: var(--color-text-muted); flex-shrink: 0; }
...
.exec-error-label { font-size: 10px; font-weight: 600; color: var(--color-error); text-transform: uppercase; letter-spacing: 0.04em; }
.exec-error-text { font-size: 11px; color: var(--color-error); word-break: break-word; line-height: 1.5; }

.exec-node-detail { padding: 8px 10px; border-top: 1px solid var(--color-border); font-size: 11px; }
.exec-llm-meta { display: flex; flex-wrap: wrap; gap: 8px; color: var(--color-text-secondary); margin-bottom: 6px; }
.exec-output-label { font-size: 10px; font-weight: 600; color: var(--color-text-muted); text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 4px; }
.exec-output-pre {
...
.exec-timing { margin-top: 6px; color: var(--color-text-muted); font-size: 10px; }

/* Toast */
.run-toast { position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); padding: 10px 20px; border-radius: var(--radius-md); font-size: 13px; cursor: pointer; z-index: 100; box-shadow: var(--shadow-lg); }
.run-toast-error { background: var(--color-error); color: #fff; }
</style>
