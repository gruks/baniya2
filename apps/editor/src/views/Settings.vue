      ...
      <div class="page-container">
        ...
        <div class="settings-section card">
          ...
          <div class="setting-row">
            <div>
              ...
              <div class="setting-desc">Connect to a local Ollama server running in the background</div>
            </div>
            <label class="toggle">
          ...
          <div class="setting-row">
            ...
            <input type="text" v-model="form.ollamaUrl" placeholder="http://localhost:11434" class="input" />
          </div>
          <div class="setting-row">
          ...
          <div class="setting-row">
            <span :class="['badge', ollamaConnected ? 'badge-success' : 'badge-muted']">
              {{ ollamaConnected ? 'Connected' : 'Offline' }}
            </span>
            <button class="btn btn-sm btn-primary" @click="refreshModels" :disabled="loadingModels">
        ...
        <div class="settings-section card">
          ...
          <div class="setting-row column">
            ...
            <input type="password" v-model="form.openaiApiKey" placeholder="sk-..." class="input full" />
            <div class="setting-hint">Used for GPT-4o, GPT-4o-mini models</div>
          </div>
          <div class="setting-row column">
            <div class="setting-label">Anthropic API Key</div>
            <input type="password" v-model="form.anthropicApiKey" placeholder="sk-ant-..." class="input full" />
            <div class="setting-hint">Used for Claude models</div>
          </div>
          <div class="setting-row column">
            <div class="setting-label">Google API Key</div>
            <input type="password" v-model="form.googleApiKey" placeholder="AI..." class="input full" />
            <div class="setting-hint">Used for Gemini models</div>
          </div>
          <div class="setting-row">
            <span :class="['badge', hasAnyCloudKey ? 'badge-success' : 'badge-muted']">
...
const hasAnyCloudKey = computed(() => 
  form.value.openaiApiKey || form.value.anthropicApiKey || form.value.googleApiKey ||
  providers.status.openai || providers.status.anthropic || providers.status.gemini
);
...
async function loadSettings() {
  try {
    ...
    form.value.defaultLocalModel = s.defaultLocalModel || 'llama3.2';
    form.value.defaultCloudModel = s.defaultCloudModel || 'gemini-1.5-flash';
...
async function refreshModels() {
  ...
  try {
    ...
    if (localModels.value.length > 0 && !localModels.value.includes(form.value.defaultLocalModel)) {
...
.setting-hint { font-size: 11px; color: var(--color-text-muted); }
.input { padding: 6px 10px; border: 1px solid var(--color-border-strong); border-radius: var(--radius-sm); font-size: 13px; background: var(--color-bg-primary); color: var(--color-text-primary); }
