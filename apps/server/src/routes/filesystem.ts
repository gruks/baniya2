...
filesystemRouter.get(
  '/list',
  validate(filesystemQuerySchema, 'query'),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      if (stat.size > 5 * 1024 * 1024) {
        return res
          .status(413)
          .json({ error: 'File too large (>5MB). Use binary download.' });
      }
...
filesystemRouter.post(
  '/write',
  validate(filesystemWriteSchema),
  async (req, res) => {
    try {
...
filesystemRouter.post(
  '/patch',
  validate(filesystemPatchSchema),
  async (req, res) => {
    try {
      ...
      if (!content.includes(oldStr)) {
        return res
          .status(409)
          .json({ error: 'oldStr not found in file', path: filePath });
      }
...
filesystemRouter.post(
  '/append',
  validate(filesystemAppendSchema),
  async (req, res) => {
    try {
...
filesystemRouter.delete(
  '/delete',
  validate(filesystemDeleteSchema, 'query'),
  async (req, res) => {
    try {
      const root = resolveRoot(req.query.root as string);
      const filePath = abs(root, req.query.path as string);
...
filesystemRouter.post(
  '/rename',
  validate(filesystemRenameSchema),
  async (req, res) => {
    try {
...
filesystemRouter.post(
  '/mkdir',
  validate(filesystemMkdirSchema),
  async (req, res) => {
    try {
